...
class PlaylistsService {
	...
	async addPlaylist({ name, owner }) {
		...
		return mapPlaylistModel(playlistResult.rows[0]).id;
	}
	...
	async getPlaylists(owner) {
		...
		const playlistResult = await this._pool.query(playlistQuery);
		return playlistResult.rows.map(mapPlaylistWithUsernameModel);
	}
	...
	async verifyPlaylistOwner(id, owner) {
		...
		const playlist = mapPlaylistModel(playlistResult.rows[0]);
	...
	async addPlaylistSong(playlistId, songId) {
		...
		return mapPlaylistSongModel(playlistSongResult.rows[0]).id;
	}
	...
	async getPlaylistSongById(id) {
		...
		const playlistSong = mapPlaylistWithUsernameModel(playlistSongResult.rows[0]);
		const songs = playlistSongResult.rows.map(mapSongModel);
	...
	async addPlaylistSongActivity(playlistId, songId, userId, action) {
		...
		if (!playlistSongActivityResult.rowCount) {
			throw new InvariantError("Gagal menambahkan aktivitas lagu playlist.");
		}

		return mapPlaylistSongActivityModel(playlistSongActivityResult.rows[0]).id;
	}
	...
	async getPlaylistSongActivities(owner) {
		...
		const playlistSongActivityResult = await this._pool.query(playlistSongActivityQuery);
		const playlistResult = mapPlaylistWithUsernameModel(playlistSongActivityResult.rows[0]);
